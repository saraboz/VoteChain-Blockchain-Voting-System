...
import { activateElection } from '../../utils/blockchainService';
...
function ActivateElection() {
  ...
  const handleActivateElection = async (e) => {
    e.preventDefault();
    ...
    if (!isConnected) {
      setError('Please connect your wallet to activate an election');
      return;
    }
    ...
    try {
      ...
      if (response.status) {
        setSuccess(`Election ${electionId} activated successfully!`);
        setElectionId(''); // Clear the input
      } else {
        setError(response.error || 'Failed to activate election');
      }
    } catch (err) {
    ...
    <div className="bg-white shadow overflow-hidden rounded-lg">
      <div className="px-4 py-3 sm:px-6 border-b border-gray-200">
        ...
        <p className="mt-1 max-w-2xl text-xs text-gray-500">
          Start an election to allow voters to cast votes.
        </p>
      </div>
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-xs text-gray-600">
              Connect your wallet to activate an election
            </p>
          </div>
          <WalletConnect />
        </div>
        ...
        <form onSubmit={handleActivateElection} className="space-y-4">
          <div>
            ...
            <input
              type="text"
              id="electionId"
              className="mt-1 block w-full pl-3 pr-10 py-1.5 text-base bg-green-50 border-green-200 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm rounded-md text-gray-800"
...
export default ActivateElection; 